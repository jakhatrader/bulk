/*
 * Proof of BULKTRADE — backend (accumulating full-history index)
 *
 * How it works (same idea as proofofhype):
 *  1. BACKFILL: walk X search history backward (newest→oldest) all the way to the
 *     FIRST ever BulkTrade-related tweet, storing every one in a persistent index.
...
// History floor. EMPTY = index everything back to the very first BulkTrade tweet
// (i.e. from @bulktrade's first post). Set a date like 2024-01-01 to limit it.
const SINCE_DATE = process.env.SINCE_DATE || '2024-05-01';
...
const TERMS = process.env.QUERY_TERMS || '(bulktrade OR @bulktrade OR $BULK)';
...
const EXCLUDE_HANDLES = (process.env.EXCLUDE_HANDLES || 'bulktrade')
...
const REDIS_KEY = process.env.STORE_KEY || 'bt:index';
...
function classify(t) {
	...
	const atV = mentions.includes('bulktrade') || low.includes('@bulktrade');
	const kw = /\bbulktrade\b/.test(low);
	const varTag = /\$bulk\b/i.test(text) || tags.includes('bulk');
